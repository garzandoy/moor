...
// Inline WelcomeModal Component
function WelcomeModal({ userId, userEmail, onComplete }) {
...
export default function ProfileClient({ profile, achievements, weekActivity, userId, userEmail }) {
  const router = useRouter();
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (!completed && !profile?.full_name) {
        setShowWelcome(true);
      }
    }
  }, [profile]);
                ...
                <div>
                  ...
                  {!profile?.full_name && (
                    <button
                      ...
                      className="text-sm bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg transition-colors mb-2"
                    >
                      Add your name →
                    </button>
                  )}
